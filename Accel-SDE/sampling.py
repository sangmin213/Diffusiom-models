...
# 논문 Alg.4
def pc_sampler(model, half_model, marginal_prob_std, diffusion_coeff, batch_size=64, num_steps=num_steps, snr = signal_to_noise_ratio, device='cuda', eps=1e-3, in_channel=1):
    '''Generate samples from score-based models with Predictor-Corrector method.'''
    t = torch.ones(batch_size, device=device)
    init_x = torch.randn(batch_size, in_channel, 28, 28, device=device) * marginal_prob_std(t)[:, None, None, None]
    time_steps1 = np.linspace(1., 0.5, num_steps//2)
    time_steps2 = np.linspace(0.5, eps, num_steps//2)

    step_size = time_steps1[0] - time_steps1[1]
    x = init_x
    with torch.no_grad():
        for time_step in tqdm(time_steps1):
            batch_time_step = torch.ones(batch_size).to(device) * time_step
            # Corrector step (Langevin MCMC)
            grad = half_model(x, batch_time_step) # score = gradient of p(x)
            ...
            g = diffusion_coeff(batch_time_step)
            x_mean = x + (g**2)[:, None, None, None] * step_size * half_model(x, batch_time_step) # (g**2) * (step size) = sigma^t - sigma^(t-\delta{t})
            x = x_mean + torch.sqrt(g**2 * step_size)[:, None, None, None] * torch.randn_like(x)

    step_size = time_steps2[0] - time_steps2[1]
    with torch.no_grad():
        for time_step in tqdm(time_steps2):
            batch_time_step = torch.ones(batch_size).to(device) * time_step
            # Corrector step (Langevin MCMC)
...
def ode_sampler(model, half_model, marginal_prob_std, diffusion_coeff, batch_size=64, 
    ...
    def score_eval_wrapper(model, sample, time_steps):
    ...
    def ode_func(t, x):
        ...
        if t < 0.5:
            return -0.5*(g**2)*score_eval_wrapper(model, x, time_steps)
        else:
            return -0.5*(g**2)*score_eval_wrapper(half_model, x, time_steps)
    
    # Run the black-box ODE solver.
    res = solve_ivp(ode_func, (1.0, 0.5), init_x.reshape(-1).cpu().numpy(), rtol=rtol, atol=atol, method='RK45')
    print(f"Number of function evaluations1: {res.nfev}")
    res = solve_ivp(ode_func, (0.5, eps), res.y[:,-1], rtol=rtol, atol=atol, method='RK45')
    print(f"Number of function evaluations2: {res.nfev}")
